...
연속형 변수(기성공정률, 외국인비율, 리커트 척도 변수)는 변수 간 단위 차이에 영향을 받는 모형에 한하여 표준화(Standardization)를 적용하였다. 로지스틱 회귀 분석에서는 표준화된 계수를 통해 변수 간 상대적 효과 크기를 비교할 수 있도록 평균 0, 표준편차 1의 Z-점수로 변환하였다. 반면 트리 기반 모델(Random Forest, XGBoost, LightGBM)은 변수 크기에 불변(scale-invariant)한 특성을 가지므로 표준화를 적용하지 않고 원본 스케일을 유지하였다. 한편 외국인비율은 전체의 50.3%가 0%로 비대칭 분포를 보이나, 해석의 일관성과 선행연구와의 비교 가능성을 위해 다른 연속형 변수와 동일한 방식으로 표준화를 적용하였다.
