...
## 표 1. 연속형 변수 군 비교 (사고 발생군 vs 미발생군)
...
> **해석**: 외국인비율·기성공정률은 사고 발생군에서 유의하게 높고(p<0.001), 정리정돈상태는 유의하게 낮다(p=0.001). 즉 현장의 물리적 특성(외국인 비율·공정 후반)이 위험 요인이고 정돈 상태가 보호 요인임을 단변량에서 이미 확인할 수 있다.

---

## 표 2. 범주형 변수 군 비교 (사고 발생군 vs 미발생군)
...
> **해석**: 외부기관 개입(감독·지원)과 사업장 구조 특성(공사규모·발주처·공사종류)은 사고 발생군에서 유의하게 다른 분포(p<0.001)를 보입니다. 외부기관 개입이 사고군에서 더 많이 나타나는 것은 고위험 현장이 감독·지원 대상으로 선택되는 구조를 시사합니다(선택편향). 반면 내부 안전관리 지표(안전조직·위원회·인증)는 군 간 차이가 없어, 형식적 관리 체계 보유만으로는 사고 예방 효과를 담보하지 못합니다.
...
> **해석**: 1,375개 사업장 중 사고 미발생이 984개(71.6%), 사고 발생이 391개(28.4%)로 약 2.5:1의 불균형 분포입니다. 이 구조 자체가 머신러닝 모델의 양성 클래스(사고 발생) 학습을 방해하므로, 본 연구는 학습셋에 한정해 SMOTENC로 합성 표본을 추가하여 불균형을 완화했습니다(테스트셋은 원본 분포 유지).
...
> - **공사종류·기성공정률**(통제변수)과 **고용노동부감독·안전보건공단지원**(외부기관)이 상위 4위를 차지합니다. 사고가 개별 안전 행동보다 **사업장 구조적 특성과 외부기관 개입**에 더 좌우된다는 점을 시사합니다.
